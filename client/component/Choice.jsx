...
function Choice({
  ...
  useEffect(() => {
    ...
    if (displayDogs.length) {
      ...
      setDogDisplay(
        <div id="choice-box">
          <div id="alt">Looking for dogs...</div>
        </div>
      );
    }
  ...
  const no = dogDisplayInfo ? (
    <Col>
      <button id="no" onClick={() => like(false)}>
        No
      </button>
    </Col>
  ) : (
  ...
  const yes = dogDisplayInfo ? (
    <Col>
      <button id="yes" onClick={() => like(true)}>
        Yes
      </button>
    </Col>
  ) : (
  ...
  const profileLink = dogDisplayInfo ? (
    <Col>
      <Link to={`/dogprofile/${dogDisplayInfo.id}`} id="view">
        View Profile
      </Link>
    </Col>
  ) : (
  ...
  return (
    <div>
      <div>
        <Row>
          <button id="settings" onClick={open}>
            Menu
          </button>
        </Row>
        <Row>
